...
BranchOps Agent is an ephemeral, read-only AI analyst for operational questions. It is available at `/reports/branchops-agent` for `Admin`, `BranchManager`, and `StockManager` users.
...
- Tool calls are audited with entity type `BranchOpsAgent`.
- Admin users can query across all branches or select one branch.
- Non-Admin users are forced to the branch in their JWT claim, regardless of any client-supplied branch id.

## Flow

1. The React page sends `POST /api/BranchOpsAgent/stream` with a message, optional branch id, and the last few in-memory messages.
2. The API creates a transient correlation id and sets AI run context to `BranchOpsAgent`.
...
Settings live under `Ai:BranchOpsAgent`:
